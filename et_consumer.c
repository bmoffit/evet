...
int main(int argc,char **argv) {
    ...
    unsigned int *coda_event_buffer = (unsigned int*)malloc(MAXBUFLEN*sizeof(unsigned int));
    ...
    if ((status =
	 et_station_create_at(id, &my_stat, stationName, sconfig, position, pposition)) != ET_OK) {
        ...
        if (readData) {
            ...
            for (j = 0; j < numRead; j++) {
                ...
                if (verbose) {
                    ...
                    if (swap) {
                        printf(" %2d/%2d: data (len = %d) needs swapping, swapped int = %d\n",
			       j, numRead, (int) len, ET_SWAP32(data[0]));
                    }
                    else {
                        printf(" %2d/%2d: data (len = %d) does NOT need swapping, int = %d\n",
			       j, numRead, (int) len, data[0]);
                    }

		    unsigned int idata;
		    for(idata = 0; idata < ((32 < (len>>2)) ? 32 : (len>>2)); idata++)
		      {
			printf("0x%08x ", swap ? bswap_32(data[idata]) : data[idata]);
			if(((idata+1) % 8) == 0)
			  printf("\n");
		      }
		    printf("\n");
		    ...
		    int handle;
		    int evstat = evOpenBuffer((char*) data, len, "r",  &handle);
		    printf("  evstat = %d\n", evstat);
		    ...
		    int PRINTOUT=1;
		    int evCount = 0;
		    while(status == 0 && status_decode > 0)
		      {
			printf("evRead(%2d): \n", evCount++);
			if((status = evRead(handle, (unsigned int *) coda_event_buffer, MAXBUFLEN)) == 0)
			  {
			    int i;
			    for (i=0; i<coda_event_buffer[0]+1; i++) {
			      if(PRINTOUT) {
				printf("0x%08x ", coda_event_buffer[i]);
				if( (i+1) % 8 == 0)
				  printf("\n");
			      }
			    }
            ...
            for (j = 0; j < numRead; j++) {
                ...
                et_event_getdata(pe[j], (void **) &data);
                printf(" Got an Event of total length %ld bytes\n",len);
		unsigned int idata;
		for(idata = 0; idata < (128); idata++)
		  {
		    printf("0x%08x ", bswap_32(data[idata]));
		    if(((idata+1) % 8) == 0)
		      printf("\n");
		  }
		printf("\n");
    ...
    if(coda_event_buffer)
      free(coda_event_buffer);
